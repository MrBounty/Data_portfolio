...
def dateTime_to_second(arr):
    ...
    Return:
        arr2 (ndarray): A 2D array of second, axis 0 is sample and axis 1 is time
    '''
    epoch = arr.min()
...
def interpol_X(X, t, nb_point=100, window_size=5):
    ...
    Args:
        ...
        nb_point (int) (optional): An integer representing the number of values to interpolate. The default value is 100.
        window_size (int) (optional): An integer representing the size of the window for the moving average smoothing function. The default value is 5.
    ...
    for i in range(X.shape[0]):
        arr2[i, :] = moving_average(np.interp(arr, t_s[i, :], X[i, :]), window_size)
    return arr2
...
def import_save_data(date, box_size_deg, resolution):
    ...
    import_and_save_train_data(time_of_interest, box_size_deg, scale, catalog)
    import_and_save_X_sub(time_of_interest, box_size_deg, scale, catalog)    